...
public class IKFun extends Application{
	...
	public void start(Stage stageIn) {
		...
		removeLineButton.setOnAction(event -> {
			if(lineList.size() != 1) {
				...
				if(index == lineList.size()-1) index--;
		...
		VBox buttonsBox = new VBox(addLineButton, splitLineButton, removeLineButton, smoothLineButton, resetLineButton, centerXLineButton, addAnimationButton, removeAnimationButton);
	...
	public void doIK(boolean isStart, double mX, double mY) {
		...
		for(int runs = 10; runs >= 0; runs--) { // I originally thought i would have to loop a few times, but since it recalculates every frame, or pixel moved, theres not enough change to see
			if(isStart) {
				sX = start.getCenterX();
				sY = start.getCenterY();
	...
	public void setLineAnchorStart(int lineIndex, double xPref, double yPref) {
		...
		lineList.get(lineIndex).setEndX(lineList.get(lineIndex).getStartX()+getRelX(lineLengthsList.get(lineIndex), angle));
		lineList.get(lineIndex).setEndY(lineList.get(lineIndex).getStartY()+getRelY(lineLengthsList.get(lineIndex), angle));
	}
	...
	public void setLineAnchorEnd(int lineIndex, double xPref, double yPref) {
		...
		lineList.get(lineIndex).setStartX(lineList.get(lineIndex).getEndX()+getRelX(lineLengthsList.get(lineIndex), angle));
		lineList.get(lineIndex).setStartY(lineList.get(lineIndex).getEndY()+getRelY(lineLengthsList.get(lineIndex), angle));
	}
