...
public class MazeViewer extends Application{
	...
	int[][] grid, prev;
	int size = 250, mapSize = 1000/size, displaySize = size*mapSize, delayTime = 0, searchDelayAddition = 0, xCreationStart, yCreationStart, xOnGridStart, yOnGridStart, xOnGridEnd, yOnGridEnd;
	LinkedList<int[]> pos;
    ...
    public void start(Stage primaryStage){
    	...
    	if((size & 1) == 0) {size++; displaySize += mapSize;}
			...
			public void handle(MouseEvent mouse) {
				if(!found) {
					if(start) {
						...
						if((xCreationStart & 1) == 0) xCreationStart = xCreationStart == 0 ? 1 : xCreationStart-1;
						yCreationStart = (int)(mouse.getSceneY()/mapSize);
						if((yCreationStart & 1) == 0) yCreationStart = yCreationStart == 0 ? 1 : yCreationStart-1;
						...
						if(mouse.getButton() == MouseButton.PRIMARY) {
							...
							if((xOnGridStart & 1) == 0) xOnGridStart--;
							yOnGridStart = (int)(mouse.getSceneY()/mapSize);
							if((yOnGridStart & 1) == 0) yOnGridStart--;
							...
							if((xOnGridEnd & 1) == 0) xOnGridEnd--;
							yOnGridEnd = (int)(mouse.getSceneY()/mapSize);
							if((yOnGridEnd & 1) == 0) yOnGridEnd--;
     ...
     public void addMazeRoute() {
		...
		curPos = pos.remove();
		if(curPos[0]-2 > 0 && grid[curPos[0]-2][curPos[1]] != 0) {possibleSides.add(0); if(curPos[2] == 0 && Math.random() < 0.25) {possibleSides.add(0);possibleSides.add(0);}}
		if(curPos[1]-2 > 0 && grid[curPos[0]][curPos[1]-2] != 0) {possibleSides.add(1); if(curPos[2] == 1 && Math.random() < 0.25) {possibleSides.add(1);possibleSides.add(1);}}
		if(curPos[0]+2 < size && grid[curPos[0]+2][curPos[1]] != 0) {possibleSides.add(2); if(curPos[2] == 0 && Math.random() < 0.25) {possibleSides.add(2);possibleSides.add(2);}}
		if(curPos[1]+2 < size && grid[curPos[0]][curPos[1]+2] != 0) {possibleSides.add(3); if(curPos[2] == 1 && Math.random() < 0.25) {possibleSides.add(3);possibleSides.add(3);}}
		if((possibleSides.size() == 0)) {addMazeRoute(); return;}
     ...
     public void addSearchRoute() {
 		...
 		curPos = pos.peek();
 		if(grid[curPos[0]-1][curPos[1]] != 0 && grid[curPos[0]+1][curPos[1]] != 0 && grid[curPos[0]][curPos[1]-1] != 0 && grid[curPos[0]][curPos[1]+1] != 0) {
 			curPos = pos.remove();
