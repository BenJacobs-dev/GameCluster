...
public class Minesweeper extends Application {
  ...
  public void createTileGrid(int sizeIn, int bombPercentIn) {
    ...
    if (bombPercentIn > 40) {
      bombPercent = 40;
    } else if (bombPercentIn < 10) {
      bombPercent = 10;
    } else {
      bombPercent = bombPercentIn;
    }
  ...
  public void setTileBomb(int w, int h) {
    ...
    if (w > 0 && h > 0) {
      tileGrid[w - 1][h - 1].isClose();
    }
    if (w > 0) {
      tileGrid[w - 1][h].isClose();
    }
    if (w > 0 && h < mapSize - 1) {
      tileGrid[w - 1][h + 1].isClose();
    }
    if (h < mapSize - 1) {
      tileGrid[w][h + 1].isClose();
    }
  ...
  public void tileClicked(ActionEvent event) {
    ...
    if (first) {
      ...
      if (w > 0 && h > 0) {
        tileGrid[w - 1][h - 1].setStartTile(true);
      }
      if (w > 0) {
        tileGrid[w - 1][h].setStartTile(true);
      }
      if (w > 0 && h < mapSize - 1) {
        tileGrid[w - 1][h + 1].setStartTile(true);
      }
      if (h < mapSize - 1) {
        tileGrid[w][h + 1].setStartTile(true);
      }
      if (w < mapSize - 1 && h < mapSize - 1) {
        tileGrid[w + 1][h + 1].setStartTile(true);
      }
